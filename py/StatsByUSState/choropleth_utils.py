...
class ChoroplethUtilities(object):
    ...
    def __init__(self, iso_3166_2_code=None, one_country_df=None, all_countries_df=None, verbose=False):
        ...
        if one_country_df is None:
            self.one_country_df = nu.load_object('us_stats_df')
        else:
...
<svg {}>
  ...
  <g
    ...
    def get_tfidf_lists(self):
        self.suggestion_list_dict = nu.load_object('suggestion_list_dict')
    ...
    def get_column_description(self, column_name):
        ...
        if column_name in self.column_description_dict:
            ...
            nu.store_objects(column_description_dict=self.column_description_dict)
  ...
  <script type="application/ecmascript" id="script-{text_id}">
		...
		scaleTextNode(pathBB, &quot;text-{text_id}&quot;);
            ...
            with open(text_file_path, 'a') as f: print(
                text_str.encode(nu.encoding_type, errors=nu.encoding_error).decode(
                    encoding=nu.decoding_type, errors=nu.decoding_error
                ), file=f
            )
    ...
    def get_google_suggestion_list(self, district_name):
        ...
        conjunctified_str = nu.conjunctify_nouns(suggestion_list)
    ...
    def create_label_line_file(self):
        ...
        for district_name, row_series in self.one_country_df[~mask_series].sort_index(axis='index', ascending=False).iterrows():
            ...
            with open(self.label_line_file_path, 'a') as f: print(
                label_line_str.encode(nu.encoding_type, errors=nu.encoding_error).decode(
                    encoding=nu.decoding_type, errors=nu.decoding_error
                ), file=f
            )
    ...
    def clean_up_district_unique_dict(self):
        ...
        self.district_unique_dict = nu.load_object('district_unique_dict')
                    ...
                    if district not in self.district_unique_dict.keys():
                        for ngram in dropdown_list:
                            if key_word in ngram:
                                ...
                                break
        nu.store_objects(district_unique_dict=self.district_unique_dict)
    
    
    
    def clean_up_district_common_dict(self):
        sorted_d, reverse_sorted_d = self.get_tfidf_lists()
        self.district_common_dict = nu.load_object('district_common_dict')
                    ...
                    if district not in self.district_common_dict.keys():
                        for ngram in dropdown_list:
                            if key_word in ngram:
                                ...
                                break
        nu.store_objects(district_common_dict=self.district_common_dict)
    ...
    def create_district_first_dict(self):
        ...
        nu.store_objects(district_first_dict=district_first_dict)
    ...
    def clean_up_suggestion_list_dict(self):
        ...
        if(set(self.one_country_df.index.tolist()) - set(self.suggestion_list_dict) == set()):
            nu.store_objects(suggestion_list_dict=self.suggestion_list_dict)
    ...
    def clean_up_district_merge_dataframe(self):
        ...
        nu.store_objects(one_country_df=self.one_country_df)
